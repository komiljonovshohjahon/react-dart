...
main() {
  ...
  group('nullable callback detection, when compiled with unsound null safety:',
      () {
    setUpAll(() {
      expect(hasUnsoundNullSafety, isTrue,
          reason: 'these tests should be running under unsound null safety');
    });
    ...
    group('isRefArgumentDefinitelyNonNullable:', () {
      group('when refs are declared in null-safe code,', () {
        test('returns false, even for non-nullable refs', () {
          expect(
              isRefArgumentDefinitelyNonNullable(
                  null_safe_refs.nonNullableElementRef),
              isFalse);
        });
        ...
        test('returns false for nullable refs', () {
          expect(
              ...
              isFalse);
          expect(isRefArgumentDefinitelyNonNullable(null_safe_refs.dynamicRef),
              isFalse);
        });
      });
      ...
      group('when refs are declared in non-null-safe code,', () {
        test('returns false', () {
          expect(
              isRefArgumentDefinitelyNonNullable(non_null_safe_refs.elementRef),
              isFalse);
          expect(
              isRefArgumentDefinitelyNonNullable(non_null_safe_refs.dynamicRef),
              isFalse);
        });
      });
    });
